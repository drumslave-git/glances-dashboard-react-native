...
- **Force-killing the exe can lose recent WebView2 profile writes.** LevelDB-backed state (service-worker registrations, `localStorage`) commits lazily; `Stop-Process -Force` moments after a write throws it away, and the next launch runs against the old state — which reads exactly like "the code change did nothing". Close the window (`CloseMainWindow()`) and let it exit before relaunching whenever the test depends on persisted state. This is how the sw.js unregistration fix looked broken three launches in a row.
- **No service worker may ever control `tauri.localhost`.** A worker's `fetch(event.request)` escapes Tauri's `WebResourceRequested` interception on the first navigation of every launch — the request hits the real network, fails (`ERR_FAILED`), and Edge's "can't reach this page" shows for ~1 s until Chromium's error-page auto-reload retries, costing ~3.5 s per launch. `public/index.html` therefore skips registration on that hostname and unregisters leftovers; keep that guard if the registration script is ever touched.
